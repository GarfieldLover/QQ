QfavUtil = {
    ...
    reloadImgSrc : function(name, src, notifyUrl) {
        var imgs = document.getElementsByName(name);
        this.log("reloadImgSrc name=" + name + "; found " + imgs.length + " imgs");
        for (var i = 0; i < imgs.length; ++i) {
            if (imgs[i].src != src) imgs[i].src = src;
            this.log("reloadImgSrc name=" + imgs[i].name + "; src=" + imgs[i].src);
            imgs[i].addEventListener("load", function(event) {
                QfavUtil._postMessageToNative(notifyUrl);
            });
        }
    },
};

QfavUtil.NOTIFY_SCHEMA = "qfenotify";